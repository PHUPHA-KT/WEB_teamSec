...
- **เรื่องหลายวัน/สัปดาห์**: ติ๊กหลายวันใน modal ได้ — เก็บเป็น `days` + `statusByDay` (สถานะแยกต่อวัน) ส่วน `day` = วันแรก ไว้ให้โค้ดเก่าอ่าน. เรื่องวันเดียวไม่มี 2 field นี้ ทำงานเหมือนเดิม
- **ตอนล่าสุดที่ทำแล้ว**: `lastEp` (ตัวเลข/ข้อความ เหมือนตอนค้าง) โชว์ป้าย "ล่าสุด ตอน N" ในงานประจำ + ทุกเรื่อง — อัปเดตเองตอนกด ✕ ตอนค้าง / เคลียร์ทุกตอน (เอาตอนที่สูงกว่า) กดป้ายแก้เองได้. เปลี่ยนสถานะรายสัปดาห์ **ไม่** บวกให้เอง (ตัดสินใจแล้ว)
- **ป้ายเว็บต้นทาง**: ตัวย่อจากโดเมนของ `link` (LZ / RIDI / KKO / TT / MRB / MD …) หน้าชื่อเรื่องในงานประจำ + ทุกเรื่อง ไม่ต้องกรอกเพิ่ม — เพิ่มเว็บใหม่ที่ `SOURCE_ABBR` ใน `tools/source.js` โดเมนที่ไม่รู้จักใช้ชื่อโดเมนย่อ
- **ย้ายเรื่องเปิดใหม่ก่อนติ๊กครบ**: ปุ่ม → งานประจำ ขึ้นทุกการ์ด ถ้ายังไม่ครบ การ์ดเก็บ `promotedId` (id งานประจำ) ขึ้นป้าย "อยู่ในงานประจำแล้ว · รอ: …" แล้วลงถังขยะเองเมื่อคนสุดท้ายติ๊ก. ถ้างานประจำตัวนั้นถูกลบ ปุ่มกลับมา
- **แก้พร้อมกัน**: `mergeById` รวมแบบ 3-way ทีละช่อง (`tools/merge.js`) — 2 คนแก้คนละช่องของเรื่องเดียวกันได้ทั้งคู่, ตอนค้าง/วัน รวมแบบเซ็ต, object รวมทีละ key, ชนช่องเดียวกัน = เครื่องที่เซฟทีหลังชนะ
- **ประวัติ**: เก็บ 30 วัน (สูงสุด 1500 รายการ) แทน 150 รายการ
